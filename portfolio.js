...
window.InkspirationsBringHomeData = Object.assign(window.InkspirationsBringHomeData || {}, {
  "roberts-poem": {
    ...
    description: "An original poem by Robert Marleton.",
    about: "A reflective writing piece shaped by weather, color, and atmosphere.",
    medium: "Original Poetry",
    products: ["print", "commission", "contact"],
    recommendations: {
      room: "Writing Room",
...
const fallbackCatalog = [
  ["soul-current", "Submersion", "Fluid Soul Series", "Digital Lettering / Ink Study"],
  ...
  ["blessed-water", "Between Waves", "Words & Expressions", "Lettering / Water Study"],
  ["infinite-love-red", "Electric Love", "Words & Expressions", "Digital Lettering"],
  ["infinite-love-violet", "Electric Love, Blue Variation", "Words & Expressions", "Digital Lettering"],
  ["you-priceless", "U R Priceless", "Words & Expressions", "Digital Lettering"],
  ["say-no-poster", "Say No to Provoke and Blame", "Words & Expressions", "Typographic Poster"],
  ["inkspirations-mark", "Inkspirations Studio Mark", "Identity & Brand", "Identity Design"],
  ["studio-blue-card", "Studio Blue Identity Card", "Identity & Brand", "Identity Design"],
  ["inkspirations-logo-card", "Inkspirations Logo Lockup", "Identity & Brand", "Identity Design"],
  ["small-town-teacher", "Small Town Teacher", "Identity & Brand", "Identity Design"]
...
function askHref(work) {
  ...
  const body = `Hi Robert, I am interested in ${work.title}. Please share the available print, original, or custom presentation options.`;
...
function primaryAction(work) {
  if (work.printAvailable && work.printHref) return { href: assetPath(work.printHref), label: "Order a Print", external: true };
...
function filtersForWorks() {
  const categories = Array.from(new Map(works.map((work) => [categoryId(work.category), work.category])).entries());
  return [{ id: "all", label: "All" }, ...categories.map(([id, label]) => ({ id, label }))];
}

function filteredWorks() {
  const term = searchTerm.trim().toLowerCase();
  const filtered = works.filter((work) => activeFilter === "all" || categoryId(work.category) === activeFilter).filter((work) => !term || workText(work).includes(term));
...
function renderFilters() {
  filterRow.innerHTML = filtersForWorks().map((filter) => `<button class="filter-chip" type="button" data-filter="${filter.id}" aria-pressed="${filter.id === activeFilter}">${escapeHtml(filter.label)}</button>`).join("");
}

function renderGallery() {
  visibleWorks = filteredWorks();
  galleryCount.textContent = `${visibleWorks.length} / ${works.length} Works`;
...
function scrollToFirstResult() {
  ...
  if (!firstCard) return;
  requestAnimationFrame(() => window.scrollTo({ top: firstCard.getBoundingClientRect().top + window.pageYOffset - 12, left: 0, behavior: "smooth" }));
}
...
function openLightbox(position) {
  ...
  modalType.textContent = `${activeIndex + 1} / ${visibleWorks.length} · ${work.medium || work.category || "Artwork"}`;
...
function moveLightbox(direction) {
  if (modal.classList.contains("open")) openLightbox(activeIndex + direction);
}
...
async function loadGallery() {
  ...
  try {
    ...
    console.warn("Using the built-in clean gallery catalog because the JSON catalog did not load.", error);
  }
}
